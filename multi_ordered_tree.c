...
//
// list the persons with the same zip code
//

void listSameZipCode(tree_node_t *root)
{
    if (root != NULL)
    {
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3)
    {
        ...
        fprintf(stderr, "  -list 'same zip code' # list the persons with the same zip code\n");
        return 1;
    }
    ...
    for (int i = 3; i < argc; i++)
    {
        ...
        if (strcmp(argv[i], "-list") == 0 && strcmp(argv[i + 1], "same zip code") == 0)
        {
            printf("List of persons with the same zip code:\n");
            listSameZipCode(roots[1]);
            continue;
        }
